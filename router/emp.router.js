...
// object reference  for express router
...
empRoute.route('/add').post(function (req, res) {
    ...
    data.save().then(emp => {
        ...
        res.status(400).json({ response: 'Unable to save a value into database' });
        res.redirect('/');
    })
});
// get request for edit employee data
...
//put request for updating existng
